              ...
              <>
                      ...
                      <Link
                        ...
                        className={`px-4 py-2 rounded-xl text-sm font-semibold tracking-wide transition-colors ${
                          isActive
                            ...
                            : "text-foreground/75 hover:text-primary hover:bg-accent-pink/15"
                        }`}
                      >
                        {link.name}
                      </Link>
                ...
                <div className="flex items-center gap-2 sm:gap-3">
                  ...
                  <button
                    ...
                    className="relative p-2 rounded-xl text-foreground/80 hover:text-primary hover:bg-accent-pink/15 transition-colors cursor-pointer"
                  ...
                  >
                    ...
                    {cartCount > 0 && (
                      <span className="absolute top-0.5 right-0.5 flex h-5 w-5 items-center justify-center rounded-full bg-primary text-[10px] font-bold text-white ring-2 ring-background">
                        {cartCount}
                      </span>
                    )}
                  </button>
            ...
            {navLinks.map((link) => (
              <Link
                ...
                className={`block px-4 py-3 rounded-xl text-sm font-semibold ${
                  pathname === link.path ? "bg-primary text-white" : "text-foreground/80 hover:bg-accent-pink/15"
                }`}
              >
                {link.name}
              </Link>
            ...
            <div className="pt-3 mt-2 border-t border-border-custom/50 flex flex-col gap-2">
              {isCustomer ? (
                ...
                <Link
                  to={loginUrl()}
                  onClick={() => setIsMobileMenuOpen(false)}
                  className="block w-full text-center px-4 py-2.5 rounded-xl text-sm font-semibold bg-primary text-white"
                >
                  Sign in
                </Link>
              )}
              ...
              <div className="flex h-full flex-col bg-background shadow-2xl border-l border-border-custom">
                ...
                <div className="flex-1 overflow-y-auto px-5 py-5">
                  {cartItems.length === 0 ? (
                    ...
                    <div className="space-y-4">
                      {cartItems.map((item) => (
                        <div key={`${item.id}-${item.size}`} className="flex gap-3 p-3 rounded-xl border border-border-custom">
                          ...
                          <div className="flex flex-1 flex-col min-w-0">
                            ...
                            <p className="text-[10px] text-muted-custom">Size: {item.size}</p>
                            <div className="mt-auto pt-2 flex items-center justify-between">
                              <div className="flex items-center border border-border-custom rounded-lg overflow-hidden">
                                <button type="button" onClick={() => updateQuantity(item.id, item.size, item.quantity - 1)} className="px-2 py-1 font-bold text-xs cursor-pointer">−</button>
                                <span className="px-2 text-xs font-semibold">{item.quantity}</span>
                                <button type="button" onClick={() => updateQuantity(item.id, item.size, item.quantity + 1)} className="px-2 py-1 font-bold text-xs cursor-pointer">+</button>
                              </div>
                              <button type="button" onClick={() => removeFromCart(item.id, item.size)} className="text-[10px] font-semibold text-primary cursor-pointer">Remove</button>
                            </div>
                          </div>
                        </div>
                      ))}
                ...
                {cartItems.length > 0 && (
                  <div className="border-t border-border-custom px-5 py-5 space-y-4">
                    ...
                    <Link to="/cart" onClick={() => setIsCartOpen(false)} className="block w-full py-3 bg-primary text-white text-center text-xs font-bold uppercase rounded-xl shine-sweep">
                      View Bag
                    </Link>
                  </div>
                )}
