...
export default function ProductForm({ categories, editingProduct, onSuccess, onCancel }: ProductFormProps) {
    ...
    return (
        <form onSubmit={handleSubmit} className="p-6 bg-white border border-border-custom rounded-2xl space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <label className="block text-[10px] font-bold uppercase mb-1">Unit Cost (INR ₹)</label>
                    <input type="number" step="1" min="0" value={costPrice} onChange={(e) => setCostPrice(e.target.value)} placeholder="800" className="w-full px-4 py-2 border border-border-custom rounded-xl text-xs"  />
                    <p className="text-[10px] text-muted-custom mt-1">Your cost to source or make this item</p>
                </div>
                <div>
                    <label className="block text-[10px] font-bold uppercase mb-1">Retail Price (INR ₹)</label>
                    <input type="number" step="1" min="0" required value={price} onChange={(e) => setPrice(e.target.value)} placeholder="1999" className="w-full px-4 py-2 border border-border-custom rounded-xl text-xs"  />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <label className="block text-[10px] font-bold uppercase mb-1">Stock Quantity</label>
                    <input type="number" step="1" min="0" required value={stockQuantity} onChange={(e) => setStockQuantity(e.target.value)} placeholder="25" className="w-full px-4 py-2 border border-border-custom rounded-xl text-xs"  />
