...
export default function ProductDetailPage() {
  ...
  if (loading) {
    return (
      <main className="flex-grow flex items-center justify-center py-24 text-xs text-muted-custom">
        Loading...
      </main>
    );
  }
  ...
  if (!product) {
    return (
      <main className="flex-grow flex flex-col items-center justify-center py-24 text-center space-y-4 px-4">
        ...
        <Link to="/products" className="px-6 py-2.5 bg-primary text-white text-xs font-semibold rounded-xl">
          Back to Collection
        </Link>
      </main>
    );
